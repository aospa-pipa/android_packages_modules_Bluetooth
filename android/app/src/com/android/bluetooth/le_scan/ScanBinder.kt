...
class ScanBinder(
    internal val adapterService: AdapterService,
    internal val scanController: ScanController,
    internal val testModeEnabled: Boolean,
) : IBluetoothScan.Stub() {

    @Volatile internal var isAvailable = true
...
private fun ScanBinder.isBluetoothOn() = adapterService.state == State.ON

@RequiresPermission(value = BLUETOOTH_PRIVILEGED, conditional = true)
private fun ScanBinder.enforcePrivilegedPermissionIfNeeded(
    settings: ScanSettings,
    filters: List<ScanFilter>,
) {
     ...
     */
    fun enforcePrivilegedPermissionIfNeeded(filters: List<ScanFilter>) = filters.forEach { filter ->
        ...
        if (filter.deviceAddress != null) {
            ...
            if (filter.addressType == BluetoothDevice.ADDRESS_TYPE_PUBLIC && filter.irk == null) {
                // Do not enforce
            } else {
...
private fun ScanBinder.hasDisavowedLocationOrHasPermission(source: AttributionSource): Boolean {
    ...
    return if (isQApp) {
        adapterService.checkCallerHasFineLocation(source, Binder.getCallingUserHandle())
    } else {
        adapterService.checkCallerHasCoarseOrFineLocation(source, Binder.getCallingUserHandle())
    }
}

@RequiresPermission(BLUETOOTH_SCAN)
private fun ScanBinder.getController(source: AttributionSource, method: String): ScanController? {
...
@RequiresPermission(BLUETOOTH_SCAN)
private fun ScanBinder.withControllerRunOnScanThread(
    source: AttributionSource,
    method: String,
