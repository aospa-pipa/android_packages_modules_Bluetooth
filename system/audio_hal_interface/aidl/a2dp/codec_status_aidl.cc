...
bool A2dpAptxAdaptiveToHalConfig(const ahal_codec_configuration& config, CodecConfiguration* codec_config) {
  ...
  aptx_adaptive_config.sampleRateHz = A2dpCodecToHalSampleRate(config.codec_config);
  LOG(ERROR) << __func__ << ": sampleRateHz " << aptx_adaptive_config.sampleRateHz;
  if (aptx_adaptive_config.sampleRateHz <= 0) {
    LOG(ERROR) << __func__ << ": Unknown aptX Adaptive sample_rate=" << config.codec_config.sample_rate;
    return false;
  }

  LOG(ERROR) << __func__ << ": Current codec channel mode = " << config.codec_config.channel_mode;
  aptx_adaptive_config.channelMode = AptxAdaptiveCodecToHalChannelMode(config.codec_config);
  ...
  LOG(ERROR) << __func__
             ...
             << " Current Codec bitspersample = " << config.codec_config.bits_per_sample;

  aptx_adaptive_config.bitsPerSample = A2dpCodecToHalBitsPerSample(config.codec_config);
  if (aptx_adaptive_config.bitsPerSample == 0) {
    LOG(ERROR) << __func__ << ": Unknown aptX bits_per_sample = " << config.codec_config.bits_per_sample
               << " aptx_adaptive_config.bitsPerSample: " << aptx_adaptive_config.bitsPerSample;
    return false;
  }
  LOG(ERROR) << __func__ << ": Aptx Adaptive bitspersample = " << config.codec_config.bits_per_sample;
  ...
  if (a2dp_config == nullptr || !a2dp_config->copyOutOtaCodecConfig(p_ota_codec_config)) {
