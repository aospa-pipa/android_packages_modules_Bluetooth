...
void bta_dm_disable_timer_cback(void* data) {
  ...
  log::warn("Device count: {}", bta_dm_cb.link_db.count);
  if (BTM_GetNumAclLinks()) {
      for (i = 0; i < bta_dm_cb.link_db.count; i++) {
        transport = bta_dm_cb.link_db.links[i].transport;
...
void bta_dm_bredr_startup() {
  ...
  if (alarm_is_scheduled(bta_dm_cb.disable_timer)) {
    ...
    if (BTM_GetNumAclLinks()) {
      for (i = 0; i < bta_dm_cb.link_db.count; i++) {
        log::warn("Device {} transport : {}", i, bta_dm_cb.link_db.links[i].transport);
        transport = bta_dm_cb.link_db.links[i].transport;
        if (BT_TRANSPORT_BR_EDR == transport) {
          btm_remove_acl(bta_dm_cb.link_db.links[i].addr,
                         transport);
